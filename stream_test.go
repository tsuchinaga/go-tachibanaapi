...
func Test_client_streamResponseToMap(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "NSのbodyをmapに変換できる",
			arg1: []byte("p_no\x024864\x01p_date\x022022.07.25-17:49:57.424\x01p_errno\x020\x01p_err\x02\x01p_cmd\x02NS\x01p_PV\x02QNSD\x01p_ENO\x0295371\x01p_ALT\x021\x01p_ID\x0220220725174500_NOV6627\x01p_DT\x0220220725\x01p_TM\x02174500\x01p_CGN\x021\x01p_CGL\x02129\x01p_GRN\x021\x01p_GRL\x0262199\x01p_ISN\x021\x01p_ISL\x023494\x01p_SKF\x021\x01p_UPF\x02\x01p_HDL\x02<TDnet>AI: マリオン(3494) i-Bond(アイボンド）サイトリニューアルのお知らせ\x01p_TX\x02<TDnet>AI: マリオン(3494) i-Bond(アイボンド）サイトリニューアルのお知らせ\\r\\r\\r\\r開示会社：マリオン(3494)\\r\\r開示書類：i-Bond(アイボンド）サイトリニューアルのお知らせ\\r\\r開示日時：2022/07/25 17:45\\r\\r\\r\\r＜引用＞\\r\\r不動産賃貸事業、不動産証券化事業を展開する株式会社マリオン(本社:東京都新宿区、代表取締役社長福田敬司)は、2022年7月25日(月)、お金第3の置き場「i-Bond」のサービスサイトをリニューアルいたしました。\\r\\r\\r"),
			want1: map[string][]string{
				"p_no":    {"4864"},
				...
				"p_SKF":   {"1"},
				"p_UPF":   {""},
				"p_HDL":   {"<TDnet>AI: マリオン(3494) i-Bond(アイボンド）サイトリニューアルのお知らせ"},
