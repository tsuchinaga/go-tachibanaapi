	...
	tests := []struct {
		...
		{name: "LoginResponseから必要な項目を取り出してSessionを作って返す",
			response: LoginResponse{
				...
				LastLoginDateTime:     LoginDateTime{Time: time.Date(2022, 2, 24, 8, 33, 17, 0, time.Local)},
		...
		{name: "LoginResponseがエラーの場合はエラーを返す",
			response: LoginResponse{
				...
				LastLoginDateTime:     LoginDateTime{Time: time.Time{}},
				GeneralAccount:        NumberBoolFalse,
				SafekeepingAccount:    NumberBoolFalse,
				TransferAccount:       NumberBoolFalse,
				ForeignAccount:        NumberBoolFalse,
				MRFAccount:            NumberBoolFalse,
				StockSpecificAccount:  SpecificAccountTypeUnspecified,
				MarginSpecificAccount: SpecificAccountTypeUnspecified,
				DividendAccount:       NumberBoolFalse,
				SpecificAccount:       NumberBoolFalse,
				MarginAccount:         NumberBoolFalse,
				FutureOptionAccount:   NumberBoolFalse,
				MMFAccount:            NumberBoolFalse,
				ChinaForeignAccount:   NumberBoolFalse,
	...
	tests := []struct {
		...
		{name: "ログイン失敗をパースして返せる",
			...
			want1: &LoginResponse{
				CommonResponse: CommonResponse{
					No:           1,
					SendDate:     RequestTime{Time: time.Date(2022, 2, 24, 10, 35, 40, 751000000, time.Local)},
					ReceiveDate:  RequestTime{Time: time.Date(2022, 2, 24, 10, 35, 40, 723000000, time.Local)},
					ErrorNo:      ErrTypeNoProblem,
					ErrorMessage: "",
					FeatureType:  FeatureTypeLoginResponse,
				},
				...
				ForeignAccount:        "",
				MRFAccount:            "",
				StockSpecificAccount:  "",
				MarginSpecificAccount: "",
				DividendAccount:       "",
				SpecificAccount:       "",
				MarginAccount:         "",
				FutureOptionAccount:   "",
